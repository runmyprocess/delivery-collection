...
(function (DELIVERY, undefined) {
   ...
   */
   ...
   /*
       ...
       @instance_name : Name of the backoffice
       @configuration (optional): Parameters of the backoffice
       Public method
   */
   DELIVERY.backoffice.create = function (instance_name, configuration) {
       ...
       if (typeof(_) == 'undefined' || typeof(_.VERSION) != 'string') {
           DELIVERY.alert.error(default_config.message[default_config.lang].setup.failure)
       ...
       if (!instance_name || !_.isString(instance_name) || _.size(_.trim(instance_name)) == 0) {
           DELIVERY.alert.error(default_config.message[default_config.lang].setup.failure)
       ...
       // Register this instance under the "DELIVERY.backoffice.instance_name" global variable
       instance_name = _.replace(_.trim(instance_name), ' ', '_')
       DELIVERY.backoffice[instance_name] = new Backoffice(instance_name, configuration)
   ...
   /*
       ...
       Object constructor
   */
   function Backoffice (instance_name, configuration) {

       if (debug == true) {
           console.log('DEBUG: New Backoffice object instance created (Name: ' + instance_name +')')
           console.log('DEBUG: (' + instance_name + ' back office) configuration:')
       ...
       if (!(this instanceof Backoffice)) {
           ...
           return new Backoffice(instance_name, configuration)
       }

       // Set the parameters
       this.name = 'DELIVERY.backoffice.' + instance_name

       // Merge user specified configuration with default configuration
       configuration = _.merge(default_config, configuration)
       ...
       this.editItem = function (action, uid) {
           ...
           var button_template = _.template('<hr><button class="gwt-Button" onclick="<%= button_action %>"><%= button_label %></button>')
           var button_html = ''
       ...
       */
       this.readItem = function (uid) {
           ...
           var widget_form = this.widget_form
           var that = this
           ...
           function success(result) {
               ...
               RMPApplication.forEachWidget( function (widget) {
                   ...
                   if (parent && _.has(parent, 'conf.id') && parent.conf.id == widget_form) {
                       ...
                       switch (widget.getType()) {
                           ...
                           case 'RMP_ListBox':
                               ...
                               object_name = JSON.parse(widget.getListOfMyVariables())[1].split('.')[0]
                               ...
                               if (widget.getType() == 'RMP_MultiSelectionListBox' || widget.getType() == 'RMP_MultiSelectionCheckBox') {
                                   // ListOfMyVariables returns an array, the first element is the label, the second the variable
                                   // If this variable exists in the record, set the widget accordingly
                                   if (record.hasOwnProperty(object_name)) {
                                       widget.setPicked(JSON.stringify(record[object_name][label]), JSON.stringify(record[object_name][variable]));
                                   ...
                                   // If this variable exists in the record, set the widget accordingly
                                   if (record.hasOwnProperty(object_name)) {
                                       widget.setSelectedValue(record[object_name][variable])
                                   } else {
                                       widget.reset()
                                   }
                               }
       ...
       */
       ...
       /*
           ...
           Syntax: DELIVERY.backoffice.my_instance_name.setReportActions("[[uid]]")
           Public method
       */
           ...
           var windowComp = $('[id="' + this.widget_form + '"]').dialog({
               ...
               open: function(event, ui){
                   if (this.editor) {
                       ...
                       var editor_data = window[this.editor[0]].getValue() || ''
                       DELIVERY.editor.setData(editor_data)
                   }
               },
       ...
       */
       function resetFormData () {
           ...
           RMPApplication.forEachWidget( function (widget) {
               ...
               var parent = widget.getParent()
               if (parent && parent.conf.hasOwnProperty('id') && parent.conf.id == widget_form) {
               ...
               if (parent && _.has(parent, 'conf.id') && parent.conf.id == widget_form) {
                   ...
                   switch (widget.getType()) {
                       ...
                       case 'RMP_ListBox':
                           ...
                           object_name = variables[1].split('.')[0]

                           // Add the label and value to the input object
                           _.set(input, object_name + '.' + label, widget.getSelectedLabel())
                           _.set(input, object_name + '.' + variable, widget.getSelectedValue())
                           break
                       ...
                       case 'RMP_MultiSelectionCheckBox':
                           ...
                           // Add the label and value to the input object
                           _.set(input, object_name + '.' + label, JSON.parse(widget.getSelectedLabel()))
                           _.set(input, object_name + '.' + variable, JSON.parse(widget.getSelectedValue()))
                           break
